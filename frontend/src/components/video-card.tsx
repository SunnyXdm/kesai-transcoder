...
export function VideoCard({ video, onTranscode, progress }: VideoCardProps) {
  ...
  const allowedQualities = useMemo(() => video.qualities.split(",").filter(Boolean), [video.qualities])

  // Initialize selected qualities only once when the component mounts
  useEffect(() => {
    if (!initialized && video.status === "pending" && allowedQualities.length > 0) {
  ...
  const getStreamUrl = () => {
    ...
    return `${API_BASE_URL}${video.m3u8Url}`
  }
  ...
  const renderQueueInfo = () => {
    if (video.status !== "queued" || typeof video.queuePosition === "undefined") return null
    ...
    return (
      <div className="mt-2 text-sm text-muted-foreground">
        ...
        <p className="mt-1 text-xs">
          Your video will be processed automatically when it reaches the front of the queue.
        </p>
      </div>
    )
  }
      ...
      <Card className="group overflow-hidden bg-card pt-0">
        <div className="relative aspect-video w-full overflow-hidden bg-muted">
          {video.blurhash && !imgLoaded && (
            <Blurhash hash={video.blurhash} width="100%" height="100%" resolutionX={32} resolutionY={32} punch={1} />
          )}
          {video.thumbnailUrl ? (
            <img
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 via-black/40 to-transparent p-4">
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className={cn("font-medium border-0", statusConfig[video.status].bg, statusConfig[video.status].color)}
              >
                <StatusIcon className={cn("mr-1.5 h-3.5 w-3.5", video.status === "processing" && "animate-spin")} />
        ...
        <CardContent className="space-y-4">
          {video.status === "pending" && allowedQualities.length > 0 && (
            <div className="space-y-3">
              <div className="text-sm font-medium text-card-foreground">Select quality presets:</div>
              <div className="grid grid-cols-2 gap-2">
                {allowedQualities.map((quality) => (
                  <button
                    key={quality}
                    onClick={() => handleCheckboxChange(quality)}
          ...
          {video.status === "processing" && (
            <div className="space-y-3">
              <div className="text-sm font-medium text-card-foreground">Transcoding Progress:</div>
              <div className="space-y-3">
                {allowedQualities.map((quality) => {
                  const progressValue = Number.parseInt(progress[`${video.id}-${quality}`] || "0")
                  return (
                    <div key={quality} className="space-y-1.5">
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">{quality}</span>
                        <span className="text-foreground font-medium">
                          {progress[`${video.id}-${quality}`] || "0"}%
                        </span>
                      </div>
          ...
          {video.status === "completed" && (
            <div className="space-y-3">
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-card-foreground">Available Qualities</h4>
                <div className="flex flex-wrap gap-2">
                  {allowedQualities.map((quality) => (
                    <Badge key={quality} variant="secondary" className="text-xs font-medium">
                      {quality}
                    </Badge>
                  ))}
                </div>
        ...
        <CardFooter>
          {video.status === "pending" && (
            <Button
              ...
              disabled={transcoding || selectedQualities.length === 0}
              className="w-full"
            >
      ...
      <Dialog open={showUrlDialog} onOpenChange={setShowUrlDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Copy the stream URL below to share or play the video.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <Input readOnly value={getStreamUrl()} onClick={(e) => (e.target as HTMLInputElement).select()} />
          </div>
          <DialogFooter className="sm:justify-start">
            <Button variant="secondary" asChild>
              <a href={getStreamUrl()} target="_blank" rel="noopener noreferrer" className="inline-flex items-center">
